...
export class GameSim {
  ...
  constructor({ mode = 'multi', level = 1, maxHumans = 9, targetEntities = 9,
    ...
    this.winnerId = null;
    this.deathOrder = 0;       // 사망 순위(placement 산출용)
  ...
  startRound() {
    ...
    this.pickups = generatePickups(this.obstacles, 18);
    this.zone = new Zone();
  ...
  findSafeSpawn() {
    for (let i = 0; i < 60; i++) {
      ...
      for (const e of this.entities) {
        if (e.alive && dist(x, y, e.x, e.y) < 230) { bad = true; break; }
      }
      if (bad) continue;
      return { x, y };
    }
    ...
    for (const b of this.bullets) {
      ...
      for (const e of this.entities) {
        ...
        if (dx * dx + dy * dy <= (b.radius + e.radius) * (b.radius + e.radius)) {
          ...
          const shooter = this.entities.find((x) => x.id === b.ownerId);
  ...
  _explodeGrenade(g) {
    ...
    for (const e of this.entities) {
      ...
      const shooter = this.entities.find((x) => x.id === g.ownerId);
  ...
  resolvePickups() {
    this.pickups = this.pickups.filter((pk) => {
      for (const e of this.entities) {
        if (e.isHuman && e.alive && dist(e.x, e.y, pk.x, pk.y) < e.radius + 14) {
